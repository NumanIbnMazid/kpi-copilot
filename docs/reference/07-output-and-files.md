...
| (not measured) | A KPI with no value is sent to PMS as an empty value with a note saying why (not due yet, not handed over, in progress), so PMS never shows a bare dash. The same text is on the KPI Summary tab before it is sent |
